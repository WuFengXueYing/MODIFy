...
def dump_scores(result_dir, hash_id, scores, converge, data, epoches, lr, gpu, module_info=None):
    with open(os.path.join(result_dir, 'experiments.txt'), 'a+') as fw:
        fw.write(hash_id + ': ' + (datetime.now()).strftime("%Y/%m/%d-%H:%M:%S") + '-' + 'data:{}'.format(data) + '-'
                 + 'epoch:{}'.format(epoches) + '-' + 'lr:{}'.format(lr) + '-' + 'gpu:{}'.format(gpu) + '\n')
        if module_info:
            fw.write(f"Module Info: {module_info}\n")
