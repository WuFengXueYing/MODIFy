...
def split_chunks(name, test_ratio=0.3, concat=False, **kwargs):
    ...
    if concat:
        ...
        chunks = get_all_chunks(name=name, **kwargs)
    ...
    print("# total chunk: {}/{} ({:.4f}%)".format(sum(train_labels) + sum(test_labels), chunk_num,
                                                  100 * ((sum(train_labels) + sum(test_labels)) / chunk_num)))
